...
protected:
    void draw(Renderer &renderer) const override
    {
        ...
        renderer.drawString(pos, "+" + std::string(content.size(), '-') + "+");
        renderer.drawString(pos + Point{0, 1}, "|" + content + "|");
        renderer.drawString(pos + Point{0, 2}, "+" + std::string(content.size(), '-') + "+");
        if (isSelected())
